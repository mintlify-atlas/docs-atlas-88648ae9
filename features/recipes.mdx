---
title: "Recipe Management: Your Personal Cookbook in Ocipe"
sidebarTitle: "Recipes"
description: "Manage your personal recipe library in Ocipe. Add, search, sort, and filter recipes by name, category, ingredients, and cooking frequency."
---
...
<Tabs>
  ...
  <Tab title="Ingredients Search">
    ...
    **Match score formula:**
    ```text
    match_score (%) = (matched_ingredient_count * 100) / total_recipe_ingredients
    ```

    The multiplication happens before the division to avoid integer truncation — the same order used in the database query. Recipes with a `0%` match are excluded entirely. The rest are sorted from highest to lowest match score, so the recipe you can most fully cook appears first.
...
<Tabs>
  ...
  <Tab title="AI Autofill">
    ...
    <Warning>
      Avoid pasting video links (YouTube, TikTok, Instagram Reels, etc.) into the autofill input. Gemini can read text-based recipe pages but cannot extract structured data from video content, and processing video URLs incurs significantly higher API costs. Use a standard recipe page URL instead.
    </Warning>
  </Tab>
